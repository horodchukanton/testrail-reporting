...
public class TestRailReportService implements ReportService {
    ...
    private Set<Result> getFailedResults() {
        return failedTests.entrySet().stream().map(s -> new Result().setCaseId(getOrCreateCaseId(s.getKey())).setComment(s.getValue()).setStatusId(Status.FAILED.getValue())).collect(Collectors.toSet());
    }
    ...
    private Set<Result> getSkippedResults() {
        return skippedTests.stream().map(s -> new Result().setCaseId(getOrCreateCaseId(s)).setStatusId(Status.SKIPPED.getValue())).collect(Collectors.toSet());
    }
    ...
    private Set<Result> getPassedResults() {
        return passedTests.stream().map(s -> new Result().setCaseId(getOrCreateCaseId(s)).setStatusId(Status.PASS.getValue())).collect(Collectors.toSet());
    }
    ...
    private Integer getOrCreateCaseId(String testCaseTitle) {

        try {
            return getCaseIdForTest(testCaseTitle);
        } catch (Exception e) {
            logger.info("Will create new case for: {}", testCaseTitle);
        }

        Case newCase = new Case()
                .setTitle(testCaseTitle);

        List<CaseField> fields = new ArrayList<>(0);
        testRail.cases().add(serviceSectionId, newCase, fields);
